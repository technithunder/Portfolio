...
const Customer = ({navigation}) => {
  ...
  const fetchAllDealers = async (
    ...
    try {
      const response = await getAllUsers('customer', pageNum, searchTerm, 10);
  ...
  const renderItem = ({item, index}) => {
    ...
    return (
      <ItemList
        ...
        item={item}
        onPress={() =>
          navigate(Routes.AdminCustomerProfile, {
            customerId: item?.BasicInfo?.id,
          })
        }
        onPressDeleteIcon={() => {
    ...
    try {
      ...
      if (response?.data?.status === 'success') {
        ...
        Toast.show({
          type: 'error',
          text1: 'Error',
          text2: response?.data?.message || 'Failed to delete customer',
          topOffset: verticalScale(IS_IOS ? 60 : 40),
        });
      }
    } catch (e) {
    ...
    <Container
      ...
      {user?.role !== 'staff' && (
        <TouchableOpacity
          ...
          onPress={() => navigate(Routes.AdminCustomerEditProfile)}>
