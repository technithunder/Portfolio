...
const EditProfile = ({navigation}) => {
  ...
  useEffect(() => {
    if (dealerId) {
      fetchSingleStaffProfile(dealerId);
    }
  }, [dealerId]);
    ...
    try {
      ...
      if (response?.data?.status === 'success') {
        ...
        setFormData({
          ...
          role: response?.data?.data?.BasicInfo?.role || 'dealer',
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const response = dealerId
        ? await updateStaffApi(dealerId, profileData)
        : await addStaffApi(profileData);
      if (response?.data?.status === 'success') {
        console.log('==>182', response?.data?.data);
        if (dealerId) {
          navigation.navigate(Routes.AdminDealersProfile, {dealerId: dealerId});
        } else {
  ...
  return (
    <Container
      title={dealerId ? editProfileName || 'Edit Dealer' : 'Add Dealer'}
      showBack={true}
      onLeftPress={() => {
        if (dealerId) {
          navigation.navigate(Routes.AdminDealersProfile, {dealerId: dealerId});
            ...
            <View
              ...
              <View
                ...
                <TouchableOpacity
                  onPress={() => {
                    if (dealerId) {
                      navigation.navigate(Routes.AdminDealersProfile, {
                        dealerId: dealerId,
                      });
                    } else {
                      navigation.navigate(Routes.DrawerStack, {
