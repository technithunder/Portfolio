...
import { passportBackPhoto } from '@/redux/Slice/VisaApplicationslice'
...
const PassportBackPage = () => {
    ...
    console.log("step2 from back page====>", step2);


    const handleConfirmPhoto = async () => {
        if (!base64Photo && !passportBack) return;
        ...
        const obj = {
            ...
            passportBack: base64Photo || passportBack,
        };


        try {
            if (base64Photo || passportBack) {
                dispatch(passportBackPhoto(base64Photo || passportBack));
                setLoading(true);
                console.log("hello=>");
    ...
    const handleUpdateBackPhoto = async () => {
        ...
        try {
            if (base64Photo || passportBack) {
                ...
                if (res.status === true) {
                    // dispatch(step2Data(res.data));
                    router.push(`/passport/${visaId}?userid=${userid}`);
                }
            } else {
    ...
    if (userid) {
        const fetchSingleUser = async (userid) => {
            try {
                ...
                setData(user.data.data.childUser.passport.back)
            } catch (err) {
    ...
    const condition = base64Photo || passportBack || data;
    return (
        <VisaApplicationLayout>
            <Box width={"100%"} height={"100%"} sx={{ display: "flex" }}>
                <StyledWrapper>
                    <StyledWrapper2>
                        <StyledWrapper3 >
                            <VerifiedIcon />
                            <Typography variant='body2' ml={"10px"}>Visa on 26 Jan, 12:00 PM</Typography>
                        </StyledWrapper3>
                        {condition ? "" :
                            <Box textAlign={"center"} mt={"50px"} width={"55%"}>
                                <Typography variant='h4' color='#393939'>
                                    The Singapore government required back page of your Passport
                                </Typography>
                            </Box>}

                        {condition ? <StyledWrapper6 >
                            <StyledWrapper4 >
                            ...
                            <Box width={"50%"} mt={2}>
                                <CommonButton
                                    ...
                                    onClick={userid ? handleUpdateBackPhoto : handleConfirmPhoto}
                                />
                                <Box mt={3}>
                                    <input
                                        type="file"
                            ...
                            <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", margin: "50px" }}>
                                <StyledWrapper5 sx={{ backgroundColor: theme.palette.common.white, }}>
                                ...
                                <StyledWrapper5 sx={{ backgroundColor: "#303030" }}>
...
export default PassportBackPage