...
export const buildQueryString = (obj: Record<string, string[]>) => {
  const queryString = Object.entries(obj)
    .flatMap(([key, values]) => values.map((value) => `${encodeURIComponent(key)}=${encodeURIComponent(value)}`)) // Encode both key and value
    .join("&");

  return queryString;
};

export function calculateStatus(amount, paymentStatus) {
  amount = parseFloat(amount); // Convert amount to a float

  if (paymentStatus === "Paid") {
      return "Paid";
  } else if (amount >= 0 && (!paymentStatus || paymentStatus === "Not Paid")) {
      return "Due";
  } else if (amount < 0) {
      return "Credit";
  }
...
export const getGuidByCustomerId = (data, customerId) => {
  return data?.length > 0 ? data.find(g => g.customerId === customerId) || null : null;
};

export function removeParamByValue(params, key, valueToRemove) {
  // Get all values for the given key
  const values = params.getAll(key);

  // Clear the key completely
  params.delete(key);

  // Re-add only the values that do not match the valueToRemove
  ...
  return params; // optional: for chaining or re-use
}